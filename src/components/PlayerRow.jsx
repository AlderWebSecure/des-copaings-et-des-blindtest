    ...
    <div style={{
      ...
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text)", display: "flex", alignItems: "center", gap: 6 }}>
          {player.name}
          {player.host && <Badge label="host" color="var(--secondary)" />}
        </div>
      </div>
      {showScore && (
        <div style={{ textAlign: "right" }}>
          <div style={{ fontWeight: 800, fontSize: 15, color: "var(--text)" }}>{player.score} pts</div>
          {delta > 0 && <div style={{ fontSize: 12, color: "var(--success)", fontWeight: 700 }}>+{delta}</div>}
        </div>
      )}
    </div>
  );
